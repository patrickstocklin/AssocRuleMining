...
def generateFrequentItemsets(Transactions, Products, minSupport):
	...
	#Dictionary with Items of size K = 1 and their supports
...
def generateNextCandidateList(ListToJoin):
	...
	sizeOfItem = len(ListToJoin[0].split(","))

	if sizeOfItem == 1:
		print ("|||Creating Joined List of K = 1|||")
		for item in ListToJoin:
			for other in ListToJoin:
				if item == other:
					pass
				else:
		...
		for item in ListToJoin:
			items = item.split(",")
			for other in ListToJoin:
				#Don't Join Same Items together
				if item == other:
					print ("Don't Join %s and %s" %(item, other))
					pass
				else:
